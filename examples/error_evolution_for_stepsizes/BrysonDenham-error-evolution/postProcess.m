...
function u = optimalControl(t)
    limit = 1/9;
    if (0<=t) && (t<=3*limit)
        u = -2/(3*limit) * (1 - t/(3*limit));
    elseif (3*limit<=t) && (t <= (1-3*limit))
        u = 0;
    elseif ((1-3*limit) <= t) && (t <= 1)
        u = -2/(3*limit) * (1 - (1-t)/(3*limit));
    else
        disp("Time is outside the domain!");
    end
end

function x1 = optimalState1(t)
    limit = 1/9;
    if (0<=t) && (t<=3*limit)
        x1 = limit*(1-(1-(t/(3*limit)))^3);
    elseif (3*limit<=t) && (t <= (1-3*limit))
        x1 = limit;
    elseif ((1-3*limit) <= t) && (t <= 1)
        x1 = limit*(1-(1-((1-t)/(3*limit)))^3);
    else
        disp("Time is outside the domain!");
    end
end

function x1 = optimalState2(t)
    limit = 1/9;
    if (0<=t) && (t<=3*limit)
        x1 = (1-(t/(3*limit)))^2;
    elseif (3*limit<=t) && (t <= (1-3*limit))
